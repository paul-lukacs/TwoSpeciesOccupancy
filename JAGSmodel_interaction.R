...
model{
  ...
  # tranformations
  for(i in 1:nSites){
    logit(psiB[i]) <- b0.psiB + error.psi[areaID[i]] # include random effects of study area on dominant species occupancy
    logit(psiAB[i]) <- b0.psiAB                        
    logit(psiA[i]) <- b0.psiA + error.psi[areaID[i]] # include random effects of study area on subordinate species occupancy
  ...
  for (i in 1:nSites){
    zB[i] ~ dbern( psiB[i] )
    zA[i] ~ dbern( zB[i] * psiAB[i] + (1-zB[i]) * psiA[i] )
  ...
  #ZZBa <- sum(zBa[])
  # derived parameters
  phi <- psiB[1]*psiAB[1]/(psiB[1]*(psiB[1]*psiAB[1]+(1-psiB[1])*psiA[1]))
