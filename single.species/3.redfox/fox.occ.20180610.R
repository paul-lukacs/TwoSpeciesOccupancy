...
EH <- y$yvv[,c(5:34)] #encouter history
nocc <- ncol(EH) #number of sampling occasions
nSites <- nrow(EH) #number of sampling sites
naive.vv <- sum(as.integer(apply(EH,1,sum, na.rm=T)>0))/nrow(EH) #naive occupancy estimate

# random effect
areaID <- as.numeric(siteCovs[,3]) #identifier for study area
narea <- max(areaID) #number of study areas
...
m$covariate <- c("p.intercept",
                 ...
                 "mean.p",
                 "mean.psi")
write.csv(m,"fox_20180610.csv")

#generate data frame with estimates for each psi beta
x <- data.frame(fox$BUGSoutput$sims.matrix)[,c("b1.psi","b2.psi","b3.psi","b4.psi","b5.psi")]
x <- round(x,2)
