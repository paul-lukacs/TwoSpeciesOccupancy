...
#  }
  ...
  # tranformations
  for( i in 1:nSites ){
    logit(psi[i]) <- b0.psi + b1.psi*area[i]*rbbt.ts[i] + b2.psi*area[i]*season[i] #+ error[areaID[i]]
      # only 20 cases of area*season==1, no linear effects.
    for( j in 1:nocc ){
