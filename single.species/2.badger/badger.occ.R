...
setwd( "/Users/pedromonterroso/Documents/Trabalho/Publications/Papers/In prep/1.spatiotemporal interactions among Iberian carnivores/3.analyses/1.rcode/occ/single.species/2.badger" )
...
nocc <- ncol(EH)
nSites <- nrow(EH)
naive.mm <- sum(as.integer(apply(EH,1,sum, na.rm=T)>0))/nrow(EH)
...
# assess model fit
plot(badger$BUGSoutput$sims.list$fit, badger$BUGSoutput$sims.list$fit.new, main = "", xlab = 
       "Discrepancy for actual data set", ylab = "Discrepancy for perfect data sets", las = 1, bty='n', pch=16, cex=0.8, 
     xlim=c(min(c(badger$BUGSoutput$sims.list$fit, badger$BUGSoutput$sims.list$fit.new)),max(c(badger$BUGSoutput$sims.list$fit, badger$BUGSoutput$sims.list$fit.new))),
     ylim=c(min(c(badger$BUGSoutput$sims.list$fit, badger$BUGSoutput$sims.list$fit.new)),max(c(badger$BUGSoutput$sims.list$fit, badger$BUGSoutput$sims.list$fit.new))))
abline(0,1, lwd = 2, col='red')

mean(badger$BUGSoutput$sims.list$fit.new > badger$BUGSoutput$sims.list$fit) # acceptable values are those that fall between 0.1 and 0.9. Extremely low (<0.1) and high (>0.9) reveal structural failure of model fit
...
m <- data.frame(badger$BUGSoutput$summary)
for(i in 1:nrow(m)){
  m$zero.perctl[i] <- ecdf_fun(badger$BUGSoutput$sims.matrix[,i],0)
...
m$covariate <- c("p.intercept",
                 ...
                 "mean.p",
                 "meanp.psi")
write.csv(m,"badger.csv")

 #generate data frame with estimates for each psi beta
x <- data.frame(badger$BUGSoutput$sims.matrix)[,c("b1.psi","b2.psi","b3.psi","b4.psi","b5.psi")]
x <- round(x,2)
