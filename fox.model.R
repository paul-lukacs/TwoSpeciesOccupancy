...
#  }
  ...
  # tranformations
  for( i in 1:nSites ){
    logit(psi[i]) <- b0.psi + b1.psi*EVI[i] + b2.psi*(EVI[i]^2) + b3.psi*rbbt.d[i] + b4.psi*rbbt.ts[i] + b5.psi*rbbt.d[i]*rbbt.ts[i] + b6.psi*d.sett[i] + b7.psi*season[i] + error[areaID[i]]
    for( j in 1:nocc ){
      logit(p[i,j]) <- b0.p + b1.p*trail[i] + b2.p*ctr[i,j] 
    }
  }
